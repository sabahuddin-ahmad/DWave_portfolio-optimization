...
def define_cqm(stocks, num_stocks_to_buy, price, returns, budget, variance):
    ...
    objective_1 = sum(-returns[i]*stocks[i] for i in range(len(stocks)))
    objective_2 = sum(variance[i][j]*stocks[i]*stocks[j] for i in range(len(stocks)) for j in range(i+1, len(stocks)))
    cqm.set_objective(objective_1 + objective_2)
